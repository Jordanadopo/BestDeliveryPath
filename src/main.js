...
$(function() {
  ...
  $('#start_btn').click(function() { 
    if(points.length >= 3) {
      ...
      alert("Veuillez ajouter svp des points de livraison aléatoires sur le map!! ☺...");
    }
  });
...
function draw() {
  if(running) {
    GANextGeneration();
    $('#status').text("Il y a " + points.length + " villes sur le map, "
                      +"la génération " + currentGeneration + " avec "
                      + mutationTimes + " mutation(s). La valeur optimale: "
                      + ~~(bestValue));
  } else {
    $('#status').text("Il y a " + points.length + " villes sur le map. ")
  }
